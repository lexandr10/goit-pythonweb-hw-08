...
async def update_contact(
    contact_id: int, contact: ContactUpdateSchema, db: AsyncSession = Depends(get_db)
):
    contact_service = ContactService(db)
...
async def search_contacts(
    ...
    return await contact_service.search_contacts(first_name, last_name, email)


@router.get("/birthday/", response_model=list[ContactResponseSchema])
async def get_contact_by_birthday(db: AsyncSession = Depends(get_db)):
    contact_service = ContactService(db)
    return await contact_service.get_contact_by_birthday()
